 ...
 * @@Modify Date: 2018-03-14 21:55:55
 * @@Function:
 */
?>
<?php
class Magiccart_Megashop_Block_Widget_Megashop extends Mage_Core_Block_Template implements Mage_Widget_Block_Interface
{
    ...
    protected function _construct()
    {
        ...
        if(!$data){
            Mage::getSingleton('core/session')->addError('Identifier '.$this->getIdentifier().' not exist in Megashop.'); 
            return;
        }
        $this->addData($data);
        parent::_construct();
    ...
    public function getProductCfg()
    {
        ...
        $options = array('limit', 'row', 'widthImages', 'heightImages', 'timer', 'action', 'category_id');
        $ajax = array();
    ...
    public function getTabActive()
    {
        ...
        if(!in_array($active, $types)){
            $active = isset($types[0]) ? $types[0] : 0;
        }
        return $active;
    }
